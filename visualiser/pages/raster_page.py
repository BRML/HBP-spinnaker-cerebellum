...
from visualiser.pages.plotter.scatter_plotter import ScatterplotChart
...
class RasterPage(AbstractPage):
    ...
    def generate_plot(self, current_time_tics, initial):
        current_time = ((current_time_tics * self.dao.machineTimeStep) / 1000.0)
        ...
        xticks = [dict(v=math.floor((position * gap) + xaxix),
                       label=math.floor((position * gap) + xaxix)) for position in range(10)]
        ...
        options = {'axis':
                ...
                'legend':
                    {'hide': False,
                     'position': {
                         'top': 20, 'right': 0, 'bottom': None, 'left': None},
                    },
                'background':
                    {
                       'chartColor': '#ffffff',
    ...
    def redraw(self, timer_tic):
        if timer_tic <= (self.dao.run_time * 1000.0) / (self.dao.machineTimeStep):
        ...
        for vert in self.vertex_in_question:
            for subvert in vert.subvertices:
                ...
                if chip.get_coords()[0] == details['coords'][0] and \
                   ...
                   processor.idx == details['coords'][2]:
                    logger.debug("Spike from %d, %d, %d, %d is from population %s" % (details['coords'][0], details['coords'][1], details['coords'][2] + 1, details['neuron_id'], vert.label))
                    self.update_data_store_with_spike(subvert, details['neuron_id'],
    ...
    def update_data_store_with_spike(self, subvertex, local_neuron_id,
        ...
        xaxix = (time_in_tics * self.dao.machineTimeStep) / 1000 #convert to ms
