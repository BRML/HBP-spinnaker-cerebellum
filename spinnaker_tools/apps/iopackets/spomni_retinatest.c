...
void timer_callback(uint key, uint payload)
{
    ...
    {
    io_printf(IO_STD, "Trying to enable retina 1 event streaming\n");

      spin1_send_mc_packet(0x11100081, 0, 1);
    }
    
    if (nIterations == 500 && chipID == 0 && coreID == 1)
    {
      spin1_send_mc_packet(0x11100080, 0, 1);
      io_printf(IO_STD, "Trying to disable retina 1 event streaming\n");
    }

    if (nIterations == 11 && chipID == 0 && coreID == 1)
    {
    io_printf(IO_STD, "Trying to enable retina 2 event streaming\n");

      spin1_send_mc_packet(0x11100101, 0, 1);
    }
    
    if (nIterations == 700 && chipID == 0 && coreID == 1)
    {
      spin1_send_mc_packet(0x11100100, 0, 1);
      io_printf(IO_STD, "Trying to disable retina 2 streaming\n");
    }
...
{
   uint x = ( payload >> 16 );
   uint y = payload & 0xFFFF;
   uint n = key & 0x3;
   x = x + ((n-1) << 7);
   
   if ( payload & 0x8000000 ) io_printf(IO_STD, "+(%03d, %03d)\n", x, y);
   else			      io_printf(IO_STD, "-(%03d, %03d)\n", x, y);
