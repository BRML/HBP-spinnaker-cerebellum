...
mflayer0_current = p.Population(16, p.SpikeSourceRemote, mflayer0_current_params, label = "mflayer0_curr_PLOT")
...
mflayer0_currset = p.Population(16, p.SpikeSourceRemote, mflayer0_currset_params, label = "mflayer0_set_PLOT")
...
iosource_params = {'min_rate': 0.05, 'max_rate': 100, 'src_type': 'glob_pois', 'sensormax': 500, 'sensormin': 0, 'listen_key': 0xFEFFFE31}
inp_iolayer_left = p.Population(size_io, p.SpikeSourceRemote, iosource_params.copy() , label = "iolayer_PLOT")
...
pop_pclayer_left= p.Population(size = size_pc, cellclass = p.IF_cond_exp, cellparams = cellparams_pclayer, label = "pclayer_PLOT")
pops_pclayer = [pop_pclayer_left]
...
pop_dcnlayer_left = p.Population(size = size_dcn, cellclass = p.IF_cond_exp, cellparams = cellparams_dcnlayer, label = "dcnlayer_PLOT")
pops_dcnlayer = [pop_dcnlayer_left]

pops_myomotor = []
myomotor_params = { 'virtual_chip_coords': {'y': 254, 'x': 254}, 
...
myomotor_left = p.Population(size_dcn, p.MyoRobotMotorControl, myomotor_params , label = "myolayer_left")
pops_myomotor.append(myomotor_left)
...
pop_grclayer_left = p.Population(size = grcsize_left, cellclass = p.IF_cond_exp, cellparams = cellparams_grclayer, label = "grclayer_PLOT") #_PLOT
...
pro_grcpcsynapses = [pro_grcpcsynapsis_left]
...
pro_mfdcnsynapsis_connector_left = p.AllToAllConnector(weights=weights_mfdcn/len(inp_mflayer_left),delays=1.0)
...
pro_pcdcnsynapsis_left = p.Projection(pop_pclayer_left, pop_dcnlayer_left, pro_pcdcnsynapsis_connector_left, target = "inhibitory" , synapse_dynamics = None, label = "pcdcnsynapsis")
