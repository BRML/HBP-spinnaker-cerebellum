...
#include "model_lif.h"
#include "recording.h"
#include "synapse.h" /* CHANGES!!! (dlester) include added */
...
/* CHANGES!!! (dlester) externs */
extern uint32_t      time;
extern current_t*    current;
extern union_decay_t excitatory_decays;
extern union_decay_t inhibitory_decays;
...
void timer_callback(uint ticks, uint null)
{
    ...
    /* CHANGES!!! (dlester) setting core-wide time */
    time = spin1_get_simulation_time();
    ...
    for(uint i = 0; i < num_populations; i++)
    {
        ...
        // DELETED dlester psp_buffer_t *psp_buffer = population[i].psp_buffer;
	
	pre_synaptic_processing_delta (population[i].num_neurons); // CHANGED!!! dlester

        for(uint j = 0; j < population[i].num_neurons; j++, neuron_count++)
        {
	  // DELETED dlester psp_buffer_t *curr_psp = &psp_buffer[j];

	  // Get excitatory and inhibitory currents from synaptic inputs
          // DELETED dlester  long long exci_current = curr_psp->exci[ticks % PSP_BUFFER_SIZE];
          // DELETED dlester  long long inhi_current = curr_psp->inhi[ticks % PSP_BUFFER_SIZE];

	  // Clear PSP buffers for this timestep
          // DELETED dlester  curr_psp->exci[ticks % PSP_BUFFER_SIZE] = 0;
          // DELETED dlester  curr_psp->inhi[ticks % PSP_BUFFER_SIZE] = 0;

            // If this neuron is not in a refractory period...
	        if(neuron[j].refrac_clock == 0)
	        {
                // ... compute the input current...
		  input = (((long long)(current[j]) - (long long)(current[SYNAPSE_TYPE_MASK | j])
			    + neuron[j].bias_current) * neuron[j].v_resistance) >> LOG_P2;
            ...
            // DELETED dlester long long exci_decay = exci_current * neuron[j].exci_decay;
            // DELETED dlester long long inhi_decay = inhi_current * neuron[j].inhi_decay;
            // Subtract the decay quantity from the current and shift back down
            // DELETED dlester long long next_exci_current = exci_current - (exci_decay >> LOG_P2);
            // DELETED dlester long long next_inhi_current = inhi_current - (inhi_decay >> LOG_P2);
            // Write decayed values to next timestep's input bins
            // DELETED dlester uint cpsr = spin1_irq_disable();
            // DELETED dlester curr_psp->exci[(ticks + 1) % PSP_BUFFER_SIZE] += (int) next_exci_current;
            // DELETED dlester curr_psp->inhi[(ticks + 1) % PSP_BUFFER_SIZE] += (int) next_inhi_current;
            // DELETED dlester spin1_mode_restore(cpsr);
            ...
            {
                record_i[population[i].num_neurons * (ticks - 1) + j] = (short) (input >> LOG_P1);
            }            // CHANGED!!! dlester WARNING!!! semantics changed input for current.
