...
SETUP_SZ = 16
PARAMS_BASE_WORDS = 3 + 7 # NEW: sensormin, sensormax, max_rate, min_rate, rbf_width, bunchshift, listen_key
...
class SpikeSourceRemote(ComponentVertex):
    """
    This class represents a Spike source object, which can represent
    a population of virtual neurons. The firing rate is controlled by
    a sensor value usually fed as a payload from outside of SpiNNaker
    """
    core_app_identifier = data_spec_constants.SPIKESOURCEREMOTE_CORE_APPLICATION_ID

    def __init__(self, atoms, contraints=None, label="SpikeSourceRemote",
            max_rate = 1, min_rate = 0.1, start = 0, duration = INFINITE_SIMULATION, 
            seed=None, gauss_width = 1.0, sensormin = -256, sensormax = 1444, src_type='rbf_pois', 
            listen_key = 0xFEFFFFF8):
        """
        Creates a new SpikeSourceRemote Object. Overlap is in sensor units (CHECK!).
        types: TODO: DOCU!
        """
        super( SpikeSourceRemote, self ).__init__(
            n_neurons = atoms,
        ...
        gauss_width = (sensormax-sensormin)/(atoms-1.)*gauss_width
        self.gauss_width = int(round(gauss_width))
        sensormin,sensormax = int(round(sensormin)),int(round(sensormax))
        self.sensormin = sensormin # these might get scaled
        self.sensormax = sensormax # at (or right before) runtime
    ...
    def getSpikeBufferSize(self, lo_atom, hi_atom, no_machine_time_steps):
        ...
        if no_machine_time_steps is None:
            return 0
        ratefactor = 1 # TODO: get a good upper estimate of the spikes we emit per tick
        OUT_SPIKE_BYTES = int(ceil((hi_atom - lo_atom + 1) / 32.)) * 4 * ratefactor
    ...
    def get_SDRAM(self, lo_atom, hi_atom, no_machine_time_steps):
        ...
        '''
        # TODO needs to be correct from here
        remoteParamsSz      = self.getParamsBytes(lo_atom, hi_atom)
        spikeHistBuffSz     = self.getSpikeBufferSize(lo_atom, hi_atom, 
                no_machine_time_steps)
        return SETUP_SZ + remoteParamsSz + spikeHistBuffSz
    ...
    def generateDataSpec(self, processor, subvertex, dao):
        ...
        single SpikeSourceRemote on one core.
        """
        ...
        appnames = { 'rbf_pois' : 'spike_source_remote_poisson_rbf.aplx',
                     'rbf_det'  : 'spike_source_remote_rbf.aplx',
                     'glob_pois': 'spike_source_remote_poisson.aplx',
                     'glob_det': 'spike_source_remote.aplx'
                    }
        # Load the expected executable to the list of load targets for this core
        # and the load addresses:
        if self.src_type in appnames.keys():
            appname = appnames[self.src_type]
        else:
            logger.error('Unknown src_type. Allowed types are: '+ str(appnames.keys()))
            exit()
    ...
    def reserveMemoryRegions(self, spec, setupSz, remoteParamsSz, \
                                                  spikeHistBuffSz):
        """
        Reserve memory regions for remote source parameters
        and output buffer.
        """
        ...
        spec.reserveMemRegion(region = SYSTEM_REGION,              \
                                size = setupSz,                    \
                               label = 'setup')
        spec.reserveMemRegion(region = REMOTE_PARAMS_REGION,       \
                                size = remoteParamsSz,             \
                               label = 'RemoteParams')
    ...
    def writeSetupInfo(self, spec, subvertex, spikeHistoryRegionSz):
        ...
        # What recording commands wereset for the parent population?
        recordingInfo = 0
    ...
    def writeRemoteParameters(self, spec, machineTimeStep, processor, 
            numNeurons):
        """
        Generate Neuron Parameter data for Remote spike sources (region 2):
        """
        spec.comment("\nWriting Neuron Parameters for %d remote sources:\n"
                % numNeurons)

        # Set the focus to the memory region 2 (neuron parameters):
        spec.switchWriteFocus(region = REMOTE_PARAMS_REGION)
        ...
        spec.write(data = self.listen_key)
#        rateParam = int(float(1000000.0 / (self.max_rate * machineTimeStep)) 
#                    * 32768.0)

        spec.write(data=int(self.sensormin),sizeof='int32')
        spec.write(data=int(self.sensormax),sizeof='int32')

        rateParam = 1000000.0 / (self.max_rate * machineTimeStep)
        spec.write(data = spec.doubleToS1615(rateParam),sizeof='s1615')
        rateParamMin = 1000000.0 / (self.min_rate * machineTimeStep)
        if rateParamMin > 32000.:
            rateParamMin = 32000.
            self.min_rate = 1000000.0 / (rateParamMin * machineTimeStep)
            logger.warn("min rate had to be rounded up to %d", self.min_rate)
        spec.write(data = spec.doubleToS1615(rateParamMin),sizeof='s1615')
#        spec.write(data = spec.doubleToS1615(self.gauss_width),sizeof='s1615')
        spec.write(data = self.gauss_width, sizeof='uint32')
        spec.write(data = self.atoms, sizeof='uint32')
        ...
        for i in range(0, numNeurons):
            ...
            rateVal     = generateParameter(self.min_rate, i)
        ...
        for (neuronId, rateVal, startVal, endVal) in slow_sources:
            isiValScaled = 1000000.0 / (rateVal * machineTimeStep)
            ...
            spec.write(data = spec.doubleToS1615(isiValScaled),   sizeof='s1615')
            spec.write(data = spec.doubleToS1615(0), sizeof='s1615')
            # time_to_spike_ticks is initialized in C code
