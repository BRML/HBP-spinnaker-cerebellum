...
bool system_header_filled (uint32_t* address, uint32_t* version, uint32_t flags)
{
  ...
  *version = address[1]; // version number extracted.
  timer_period = address[2];
  
  log_info("magic = %08x, version = %d.%d, timer period = %u", address[0],
           address[1] >> 16, address[1] & 0xFFFF, timer_period);
  return (true);
}
                         ...
                         uint32_t *spike_history_recording_region_size, uint32_t *neuron_potentials_recording_region_size, uint32_t *neuron_gsyns_recording_region_size)
{
  ...
  system_word = address[0];
  
  // Read recording region sizes
  *spike_history_recording_region_size = address[1];
  *neuron_potentials_recording_region_size = address[2];
  *neuron_gsyns_recording_region_size = address[3];
