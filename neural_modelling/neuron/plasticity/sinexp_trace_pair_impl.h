#ifndef SINEXP_TRACE_PAIR_IMPL
#define SINEXP_TRACE_PAIR_IMPL
...
#define STDP_TRACE_FIXED_POINT 16 // was 11
...
#define STDP_TRACE_TO_INPUT_SHIFT_LEFT (16 - STDP_TRACE_FIXED_POINT) // was 15
...
#define STDP_TRACE_TAU_MINUS_SIZE 16
...
{
  return (pre_synaptic_trace_entry_t){ .r1 = 0 }; //STDP_TRACE_FIXED_POINT_ONE };
}
...
//  io_printf(IO_BUF,"\tgot postsyn spike at time %u\n", spike_time);  
  // No need to keep track of >1 post-synaptic spikes
  return (post_synaptic_trace_entry_t){ .o1 = 0 }; //last_event_trace.o1 };
}
//---------------------------------------
...
{
/*  // Get time since last spike, delay by given SinAdd delay
  uint32_t delta_time = spike_time - last_event_time - plasticity_region_data.t_delay;
 ...
 */
//  int32_t new_r1_trace = STDP_TRACE_FIXED_POINT_ONE; // last_event_trace.r1 + 
 
//  plastic_runtime_log_info
//  io_printf(IO_BUF,"\tgot presyn spike at time %u\n", spike_time);
  ...
  return last_event_trace; //(pre_synaptic_trace_entry_t){ .r1 = new_r1_trace };
}
//---------------------------------------
...
static inline uint32_t stdp_trace_rule_get_final_weight(deferred_update_state_t new_state, uint32_t old_weight)
{
  ...
  int32_t scaled_potentiation = plasticity_fixed_umul32(new_state.potentiation, plasticity_region_data.a2_plus, STDP_TRACE_FIXED_POINT + (ring_buffer_to_input_left_shift - STDP_TRACE_TO_INPUT_SHIFT_LEFT) );
  int32_t scaled_depression = plasticity_fixed_umul32(new_state.depression, plasticity_region_data.a2_minus, STDP_TRACE_FIXED_POINT + (ring_buffer_to_input_left_shift - STDP_TRACE_TO_INPUT_SHIFT_LEFT) );
  ...
  plastic_runtime_log_info(IO_BUF,"\told_weight:%u, potentiation:%d, scaled_potentiation:%d, depression:%d, scaled_depression:%d,\n\tnew_weight:%d, max_weight:%d, min_weight:%d\n", 
    old_weight, new_state.potentiation, scaled_potentiation, new_state.depression, scaled_depression, new_weight,plasticity_region_data.max_weight,plasticity_region_data.min_weight);
  
  return (uint32_t)new_weight;
}
//---------------------------------------
// post before pre -> no need for an update
...
{
//  use(&event_trace);
//  io_printf(IO_BUF,"presyn spike apply\n");
  int32_t potentiation = previous_state.potentiation + STDP_TRACE_FIXED_POINT_ONE;
/*
  ...
  deferred_update_state_t previous_state)
{
  ...
  // Get time of event relative to last pre-synaptic event, subtract given SinAdd delay
  uint32_t time_since_last_pre_event = event_time - last_pre_synaptic_event_time - plasticity_region_data.t_delay;
  int32_t decay = STDP_TRACE_DECAY_LOOKUP_TAU_PLUS(time_since_last_pre_event);
//  int32_t decayed_r1_trace = STDP_TRACE_FIXED_MUL_16X16(last_pre_synaptic_event_trace.r1, exponential_decay);
//  int32_t decayed_r1_trace = STDP_TRACE_FIXED_MUL_16X16(STDP_TRACE_FIXED_POINT_ONE, exponential_decay);

   // Add this to current potentiation total
  int32_t depression = previous_state.depression + decay;

  plastic_runtime_log_info(IO_BUF,"\t\t\ttime_since_last_pre_event (sans delay) =%u, decayed_r1_trace=%d, depression=%d\n", 
    time_since_last_pre_event, decay, depression);
